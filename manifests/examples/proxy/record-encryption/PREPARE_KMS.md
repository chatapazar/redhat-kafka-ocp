...
For this example, you can choose between HashiCorp Vault&#8482; or AWS KMS&#8482;.
...
Note: these examples are prepared against HashiCorp Vault 1.18.
...
4. Create a variable containing the Vault Transit Url.  This will be used later on.
   ...
   ```
5. Edit `yaml/proxy/02.KafkaProtocolFilter.encryption.yaml`. Uncomment the configuration for Vault. Update
   `vaultTransitEngineUrl` to refer to URL of the transmit engine.  The url is formed like this:
   `https://<vault server>:8200/v1/<vault namespace(s)>/transit`
...
3. Install Vault using Helm
   ...
   helm install vault hashicorp/vault --create-namespace --namespace=vault --version v0.31.0 --values kms-setup/vault/helm-dev-values.yaml --set server.dev.devRootToken=${VAULT_TOKEN} --wait
   ```
...
6. Create a variable containing the Vault Transit Url.  This will be used later on.
   ...
   ```   
7. Edit `yaml/core/proxy/02.KafkaProtocolFilter.encryption.yaml`. Uncomment the configuration for Vault.

8. To conclude follow the steps below to [Create the Vault Policy and Filter Vault Token](#create-the-vault-policy-and-filter-vault-token)

## Create the Vault Policy and Filter Vault Token

1. Create a Vault Policy for the Encryption Filter:
   ```sh
   vault policy write proxy_encryption_filter_policy kms-setup/vault/proxy_encryption_filter_policy.hcl
2. Create a Vault Token which will be used by the Encryption Filter:
   ```bash
   vault token create -display-name "proxy encryption filter" -policy=proxy_encryption_filter_policy -no-default-policy -orphan -field=token > vault.encryption.token
   ```
3. Prepare a secret containing the Encryption Vault Token.
   ```sh
   oc create secret generic proxy-encryption-kms-secret -n my-proxy --from-file=encryption-vault-token.txt=vault.encryption.token --dry-run=client -o yaml > yaml/core/proxy/07.Secret-proxy-encryption-kms-secret.yaml 
   ```
...
Note: these examples are prepared against AWS CLI 2.31.20

### Configuring an Application Identity

1. Define a variable containing your AWS account id.
   ```sh
   AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
   APPLICATION_USER_NAME=kafkaProxyUser
   ```

2. Create an AWS IAM for use by Record Encryption:
   ```sh
   ...
   oc create secret generic proxy-encryption-kms-secret -n my-proxy --from-file=accessKeyId.txt=<(jq -r .AccessKey.AccessKeyId access_key.json) --from-file=secretAccessKey.txt=<(jq -r .AccessKey.SecretAccessKey access_key.json) --dry-run=client -o yaml > yaml/core/proxy/07.Secret-proxy-encryption-kms-secret.yaml 
   ```

3. Create an `AliasBasedIAMPolicy` granting permissions to use keys aliased `alias/KEK_*`.
   ```sh
...
4. Attach the policy to the application identity.
   ```sh
...
5. Edit `yaml/core/proxy/02.KafkaProtocolFilter.encryption.yaml`. Uncomment the configuration for AWS.  Adjust the 
   `endpointUrl` and `region` to point at the AWS service endpoint you use.
...
vault token revoke $(cat vault.encryption.token)
vault policy delete streams_for_apache_kafka_proxy_encryption_filter_policy
```
